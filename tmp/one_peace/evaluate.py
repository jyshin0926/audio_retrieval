...
def parse_args():
    parser = argparse.ArgumentParser(description='Run evaluation with custom configuration.')
    parser.add_argument('--config_dir', type=str, default="../../run_scripts", help='Directory containing the configuration files.')
    parser.add_argument('--config_name', type=str, default="evaluate",help='Path to the model.')
    parser.add_argument('--path', type=str, default="/content/drive/MyDrive/2024dcase/task8_retrieval/pretrained_chkpoint/finetune_al_retrieval_onepiece.pt", help='Name of the configuration to use.')

    parser.add_argument('--results_path', type=str, default="../../results/dcase_evaluation",help='Path to save results.')
    parser.add_argument('--task_name', type=str, default="audio_text_retrieval", help='Name of the task.')
    parser.add_argument('--model_name', type=str, default="one_peace_retrieval",help='Name of the model.')
    parser.add_argument('--gen_subset', type=str, default="/content/drive/MyDrive/2024dcase/task8_retrieval/dcase2023-audio-retrieval/data/evaluation_dataset/test",help='Subset of the data to evaluate on.')
    parser.add_argument('--batch_size', type=int, default=4, help='Batch size for evaluation.')
    return parser.parse_args()

def setup_parser():
    """Set up argparse parser to handle command-line arguments."""
    parser = argparse.ArgumentParser(description='Convert shell variables to Python dictionary for model configuration.')
    parser.add_argument('--data', type=str, default='/content/drive/MyDrive/2024dcase/task8_retrieval/dcase2023-audio-retrieval/data/evaluation_dataset/retrieval_audio_metadata.csv',
                        help='Path to the data file.')
    parser.add_argument('--selected_cols', type=str, default='sound_id,file_name,keywords,start_end_samples',
                        help='Comma-separated column names.')
    parser.add_argument('--valid_file', type=str, default='../../dataset/clotho/evaluation_texts.json',
                        help='Path to the validation file.')
    parser.add_argument('--bpe_dir', type=str, default='../../utils/BPE',
                        help='Path to the BPE directory.')
    return parser

def create_model_overrides(args):
    """Create a dictionary for model overrides based on the parsed arguments."""
    model_overrides = {
        'task': {
            'data': args.data,
...
def cli_main():
    ...
    cfg_name = '/content/drive/MyDrive/2024dcase/task8_retrieval/ONE-PEACE/one_peace/run_scripts/{}.yaml'.format(args.config_name)
    cfg_path = os.path.join(cfg_dir, cfg_name)
    # command_overrides = get_args().overrides
    command_overrides = create_model_overrides(args)
